		...
		<form>
			<div class="flex flex-col gap-[24px] text-[14px]">
				...
				<div class="flex flex-col gap-[36px]">
					...
					<Input name="from" label="From" placeholder="Enter your name here to send your wishes." />
					{#if !isLoading}
						<button
					...
					{:else}
						<button
							...
							on:click={toggleLoading}><span class="rotate">{@html loading}</span>LOADING...</button
						>
					{/if}
				</div>
			</div>
